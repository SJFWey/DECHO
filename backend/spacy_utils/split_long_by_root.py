...
import string
import warnings
from .load_nlp_model import init_nlp, SPLIT_BY_CONNECTOR_FILE
...
def split_long_sentence(doc):
    ...
    for i in range(1, n + 1):
        for j in range(
            max(0, i - 100), i
        ):  # limit search range to avoid overly long sentences
...
def split_long_by_root_main(nlp):
    ...
    punctuation = (
        string.punctuation + "'" + '"'
    )  # include all punctuation and apostrophe ' and "
